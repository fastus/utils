...
import {
	...
	customerObject,
	operatorObject
} from "abl-constants/build/objects";
import {daysRunning, timeFormat, dateFormat, startTime} from "abl-constants/build/date";
...
describe("Message", () => {
	describe("#prepare", () => {
		...
		it("prepare with loaded data from constants/objects", () => {
			...
			assert.equal(res.booking.count, 3);
			assert.deepEqual(res.transaction, {subtotal: "$620.00", total: "$685.15", tax: "$0.15", fee: "$65.00", coupon: ""});
		...
		it("prepare with changed objects (if statements)", () => {
			...
			data.event.startTime = moment(startTime).day(-2);
